# handlers/search.py (полностью исправленный код)
...
async def back_to_filters(callback: CallbackQuery, state: FSMContext):
    ...
    if not query:
        await callback.answer("Сначала введите поисковый запрос", show_alert=True)
        return
...
async def process_city_input(message: Message, state: FSMContext):
    ...
    if len(cities) == 1:
        ...
        # Убираем ReplyKeyboard если он есть
    ...
    variants = {str(c.get("id")): c.get("text", c.get("name")) for c in cities[:10]}
...
async def process_location(message: Message, state: FSMContext):
    ...
    await message.answer("🔍 Ищу город по координатам...", reply_markup=ReplyKeyboardRemove())

    # Здесь должна быть функция определения города по координатам
    # Пока заглушка
    await message.answer(
...
async def show_full_vacancy(callback: CallbackQuery, state: FSMContext):
    ...
    if isinstance(vacancy, dict):
        ...
        salary = _format_salary_obj(vacancy)
        desc = _hh_html_to_text(getattr(vacancy, "description", ""))
        requirement = _hh_html_to_text(getattr(vacancy, "requirement", ""))
...
async def subscribe_from_search(callback: CallbackQuery, state: FSMContext):
    ...
    if count >= Config.MAX_SUBSCRIPTIONS_PER_USER:
        await callback.answer(f"⚠️ Максимум {Config.MAX_SUBSCRIPTIONS_PER_USER} подписок", show_alert=True)
        return
